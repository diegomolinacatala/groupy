...
import { Check, Copy, CornerDownLeft, Lock } from "lucide-react";
import {
  claimCloudMember,
...
import { StrengthsPicker } from "@/components/setup/steps";
...
// they are (claiming binds their anonymous session to that row), declares
// their strengths, and lands on the dashboard.
...
export function WhoAreYouScreen({ preview }: { preview: ProjectPreview }) {
  ...
  // After the strengths step the server component re-renders with the
  // now-claimed session and swaps this screen for the dashboard.
  const handleFinish = async (skipStrengths = false) => {
    if (!claimedId || finishing) return;
    setFinishing(true);
    if (!skipStrengths && strengths.length > 0) {
      await setCloudMemberStrengths({ memberId: claimedId, strengths });
    }
    router.refresh();
  };
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-canvas">
      ...
      <main className="flex flex-1 items-start justify-center px-5 pt-[10vh]">
        {claimedId ? (
          <div key="strengths" className="animate-rise w-full max-w-md">
            <p className="type-overline mb-3">Tus fortalezas</p>
            ...
            <div className="mt-8">
              <StrengthsPicker
                value={strengths}
                onChange={(updater) => setStrengths(updater)}
          ...
          <div key="who" className="animate-rise w-full max-w-md">
